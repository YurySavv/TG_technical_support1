...
class AdminRepository:
    ...
    def register_admin(self, user_id, username, admin_level):
        registration_timestamp = datetime.now()
        query = "INSERT INTO Admins (user_id, username, admin_level, registration_timestamp) VALUES (?, ?, ?, ?);"
        self.conn.execute(query, (user_id, username, admin_level, registration_timestamp))
        self.conn.commit()
...
class InstructionRepository:
    ...
    def add_instruction(self, program_id, keywords, instruction_text, user_comment, approval_status, rating):
        modification_timestamp = datetime.now()
        query = "INSERT INTO Instructions (program_id, keywords, instruction_text, user_comment, approval_status, rating, modification_timestamp) VALUES (?, ?, ?, ?, ?, ?, ?);"
        self.conn.execute(query, (program_id, keywords, instruction_text, user_comment, approval_status, rating, modification_timestamp))
        self.conn.commit()
...
class UserSuggestionRepository:
    ...
    def add_user_suggestion(self, user_id, suggestion_text):
...
class OperatorSessionRepository:
    ...
    def close_operator_session(self, user_id):
...
# instruction_repo.add_instruction(1, "установка", "Инструкция по установке программы", "Нет комментариев", 2, 4.5)
# suggestion_repo.add_user_suggestion(1, "Пожелание пользователя")
# operator_session_repo.set_operator_session(1, 1)
